...
set CP=%CP%;%L%digitalcheck.jar
set CP=%CP%;%L%com4j.jar
set CP=%CP%;%L%jna-4.1.0.jar
set CP=%CP%;%L%jna-platform-4.1.0.jar
set CP=%CP%;%L%jnaerator-0.12.jar
set CP=%CP%;%L%jnaerator-runtime-0.10.jar
set CP=%CP%;%L%DccLibrary.jar
set CP=%CP%;%L%micrtools-1.0-beta1.jar

::Checking for RDM device
@echo 1
FOR /F "tokens=3" %%C in ('find /c "rdmConfiguration" .\config.xml') DO (set RDM=%%C)

if (%RDM%) EQU (1) (
	@echo RDM detected
	set CP=%CP%;%L%rdm.jar
) else (	
	@echo RDM not detected
)
...
"%JAVA%java" %DBG_OPTIONS% -cp %CP%  com.cinet.virtualterminal.bridge.VirtualTerminalBridge